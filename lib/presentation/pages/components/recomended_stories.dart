...
class RecommendedStories extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Obx(
      () {
        ...
        return Obx(
          () => Column(
            children: homeController.stories.asMap().entries.map(
              (entry) {
                int index = entry.key;
  ...
  void syncLikedStatus() {
    everAll(
      [homeController.stories, favoritesController.favoriteItems],
      (_) {
        likedStatus.clear();
        likedStatus.addAll(homeController.stories.map(
          (story) {
